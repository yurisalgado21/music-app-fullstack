...
const saveFavoriteSongs = (favoritesSongs: SongType[]) => localStorage
.setItem(FAVORITE_SONGS_KEY, JSON.stringify(favoritesSongs));
...
export const removeSong = (song: SongType): Promise<'OK'> => new Promise((resolve) => {
    const favoritesSongs = readFavoriteSongs();
    saveFavoriteSongs(favoritesSongs.filter((s) => s.trackId !== song.trackId));
    simulateRequest(SUCCESS_STATUS)(resolve);
});